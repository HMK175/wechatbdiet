...
- 依赖云函数 `rank` 与集合 `leaderboard_daily`，用户资料来自集合 `users`（云函数 `profile` 写入）。
...
- 页面加载或点击“刷新榜单”时调用 `rank` 的 `get`，传入 `date`；云函数会兼容 `data.date` 与顶层 `date` 两种文档结构并扁平化返回。
...
- **集合**：云数据库中需有 `leaderboard_daily`（排行榜按日期存储）、`users`（用户昵称、头像，由 profile 页保存后写入）。
